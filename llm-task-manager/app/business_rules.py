...
- BR-04: Sprint can only be closed if 100% of its stories are done
"""
...
class BusinessRulesEngine:
    ...
    def can_close_sprint(stories: list[Story]) -> tuple[bool, str]:
        ...
        A sprint can only be closed if 100% of its stories are at status 'done'.
        Stories in any other status (backlog, todo, in_progress, in_review)
        must be moved to 'done' or removed from the sprint before closing.
        ...
        if non_done_stories:
            story_list = ", ".join([f"{s.id} ({s.status})" for s in non_done_stories])
            return False, f"{len(non_done_stories)} stories are not done: {story_list}"
    ...
    def validate_sprint_closure(stories: list[Story], force: bool = False) -> bool:
        ...
        All stories must be in 'done' status. Use force=True to bypass.

        Args:
            stories: List of stories in the sprint
            force: If True, allow closing even with non-done stories
        ...
        if not can_close:
            raise BusinessRuleViolation(
                ...
                "Move all stories to 'done' or remove them before closing, or use force=true."
            )
        return True
